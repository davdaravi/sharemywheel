        ...
        try
        {
            ...
            else
            {
                    ...
                    if(isset($request['photo']))
                    {
                        ...
                        {
                           $ridesData=$ridesData->where('users.profile_pic','!=','/images/default.png'); 
                        }
                    }
  ...
  //          DB::getQueryLog()
    //    );
            ...
            foreach ($ridesData as $key => $value) {
                ...
                $newrideData = array();
                $message='<a href="ridedetail/'.$value->sourcename.'_'.$value->destinationname.'_'.$value->rideId.'">';
                ...
                $message1='<a href="ridedetail/'.$value->sourcename.'_'.$value->destinationname.'_'.$value->rideId.'">';
                ...
                $message2='<a href="ridedetail/'.$value->sourcename.'_'.$value->destinationname.'_'.$value->rideId.'">';
    ...
    public function getRideDetail($id)
    {
        try
        {
            ...
            return view('rideDetails',['rideDetail'=>$rideDetails,'totalRide'=>$totalRide,'loginDate'=>$loginDate,'ridePreference'=>$ridePreference]);
        }
        catch(\Exception $e)
        {
            \Log::error('carList function error'.$e->getMessage());
   ...
   //function call when click on url of ridelist
    ...
    public function getRideOffer($id)
    {
        try
        {
            ...
            return view('rideOfferOwner',['rideDetail'=>$rideDetails,'totalRide'=>$totalRide,'loginDate'=>$loginDate,'bookedUserDetail'=>$bookedUserDetail,'ridePreference'=>$ridePreference]);
        }
        catch(\Exception $e)
        {
                    ...
                    if(count($walletRecord)>0)
                    {
                        if($walletRecord[0]->amount>$totalFinal)
                        {
                            ...
                            if($status>0)
                            {
                                ...
                                $insertAdminWallet=array("rideId"=>$rideid,"userId"=>session('userId'),"amount"=>$tax,"bookType"=>"book");
    ...
    public function bookCcavenu()
    {
        try
        {
            ...
            $data['amount']=400;
            $data['currency']="INR";
            $data['redirect_url']="http://www.smw.com/successPayment";
            $data['cancel_url']="http://www.smw.com/cancelPayment";
            $data['language']="EN";
            $data['merchant_param1']=25;
            $working_key='5484B339D7D55BA534785AD374544F46';
            $access_code='AVUK64DC48BJ59KUJB';
            $merchant_data="";
            ...
            return view('ccpayment',['encrypt'=>$encrypted_data,'access'=>$access_code]);
